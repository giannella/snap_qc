...
**`utilities_sua` (2026-08-22)**: the frame carries a 3-level SUA tier (0 none / 1 below the state-year anchor minus $200 / 2 at or above), computed in `features.R` (`add_sua_tier`, anchor = `sua_anchor` = mode of positive utilities per state-year) and written by the munging script; rebuilt 2026-08-22 with every other column bit-identical to the 08-13 frame (`archive_data/reg_model_data_pre_sua_tier_2026-08-22.rds`). It is a v1 definition under SUA-structure expert review; `features.R` is the ONLY place to change it (the workbook's Excel formula in `make_input_workbook.py` mirrors it and the validation gate enforces the match). It is in the frame, not yet in the shipped delivery vocabulary: the one-year-ahead study (`methods/v250_benchmark_2024_utilrel/`) cleared its bars, the staged lists are in `methods/v250_candidate_lists_utilsua/`, and era-2 replication runs on whatever definition `features.R` carries at launch.
