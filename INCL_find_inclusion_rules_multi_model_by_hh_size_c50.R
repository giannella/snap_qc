...
# ──────────────────────────────────────────────────────────────────────────────
# RuleFit for SNAP review targeting: INCLUSION criteria — C5.0 variant
#
...
#
# Uses {C50} instead of {pre} or {xrf}. C5.0 produces a compact, directly-pruned
# ruleset (5–30 rules per trial) rather than a large pool filtered by lasso. Use
# C50_TRIALS > 1 to run a boosted ensemble and pool rules across trials.
#
# Key difference: FIT_OBJECTIVE = "dollars" here means case weights proportional
# to error dollar amounts (C5.0 is a classifier, not a regressor).
# ──────────────────────────────────────────────────────────────────────────────
...
OBJECTIVE     <- "dollars"   # net recall basis: "counts" or "dollars"
FIT_OBJECTIVE <- "counts"    # "counts" = unweighted; "dollars" = weight errors by dollar amount
...
# C5.0-specific knobs ──────────────────────────────────────────────────────────
# C50_TRIALS    : boosting rounds. 1 = single compact ruleset. 10-25 = more rule
#                 diversity at the cost of runtime. Start at 1, raise if the net
#                 runs out of candidate rules before hitting your recall floors.
# C50_CF        : confidence factor (0-1). Higher = less pruning = more, looser
#                 rules. 0.25 is the C5.0 default. Try 0.10 for very tight rules
#                 or 0.50 to keep more candidates.
# C50_MIN_CASES : minimum training cases a rule must cover. Raise to suppress
#                 micro-rules; lower to allow rules on rare sub-populations.
C50_TRIALS    <- 100
C50_CF        <- 0.9
C50_MIN_CASES <- 5
...
ERROR_TAG <- paste(sort(setdiff(unique(as.character(focal_df$error_status)), "no_error")),
                   collapse = "_")
out_file <- function(stem) file.path(out_dir, sprintf("C50_%s_%s.csv", ERROR_TAG, stem))
...
# Parse C5.0 text output into a tidy rule data frame.
# Handles both single-trial ("Rule 1:") and boosted ("Rule 1/1:") output formats.
# C5.0's confidence [value] in each rule is the Laplace-corrected precision on
# training data — we use it as the importance proxy.
parse_c50_rules <- function(fit_obj, positive_class = "error") {
  txt <- fit_obj$output
  if (!nzchar(trimws(txt))) {
    cat("  [parse_c50_rules] fit$output is empty\n")
    return(tibble())
  }

  # Normalize Windows line endings; trim each line so indentation (common in
  # boosted output) doesn't break the header regex.
  lines <- trimws(strsplit(gsub("\r\n", "\n", txt), "\n")[[1]])

  rule_idx <- grep("^Rule [0-9]+(/[0-9]+)?:", lines)
  if (length(rule_idx) == 0) {
    cat("  [parse_c50_rules] no rule headers matched; first 40 lines of fit$output:\n")
    cat(paste(head(lines, 40), collapse = "\n"), "\n")
    return(tibble())
  }

  rule_end <- c(rule_idx[-1] - 1L, length(lines))

  results <- lapply(seq_along(rule_idx), function(i) {
    block <- lines[rule_idx[i]:rule_end[i]]
    block <- block[nzchar(block)]
    if (length(block) < 2) return(NULL)

    hm      <- regmatches(block[1], regexec("\\((\\d+)(?:/(\\d+))?", block[1]))[[1]]
    n_cover <- if (length(hm) >= 2) as.integer(hm[2]) else NA_integer_
    n_miscl  <- if (length(hm) >= 3 && nzchar(hm[3])) as.integer(hm[3]) else 0L

    arrow_i <- grep("->\\s*class", block)
    if (length(arrow_i) == 0) return(NULL)

    am <- regmatches(block[arrow_i[1]],
                     regexec("->\\s*class\\s+(\\S+)\\s*\\[([0-9.]+)\\]",
                             block[arrow_i[1]]))[[1]]
    if (length(am) < 3 || am[2] != positive_class) return(NULL)
    confidence <- as.numeric(am[3])

    conds <- block[2:(arrow_i[1] - 1)]
    conds <- conds[nzchar(conds)]
    if (length(conds) == 0) return(NULL)

    # C5.0 uses bare "=" for factor levels; eval() needs "==".
    # Negative lookbehind/lookahead avoids touching <=, >=, ==, !=.
    conds <- gsub("(?<![<>!=])=(?!=)", "==", conds, perl = TRUE)

    tibble(
      rule_text       = paste(conds, collapse = " & "),
      confidence      = confidence,
      n_cover         = n_cover,
      n_misclassified = n_miscl,
      imp             = confidence
    )
  })
...
run_for_hh <- function(focal_df, hh_label) {
  ...
  # C5.0 is a classifier; "dollars" mode upweights error cases by dollar amount.
  # For "counts" we pass NULL — passing rep(1, n) triggers C5.0's "relative case
  # weighting" path even when weights are equal, which suppresses split-finding.
  case_wts <- if (FIT_OBJECTIVE == "dollars") ifelse(ie, pmax(md_dollars, 1), 1) else NULL

  y_train <- factor(ifelse(ie, "error", "clean"), levels = c("error", "clean"))
  x_train <- model_data[pv]

  # Coerce logical/integer binary columns to numeric; C5.0 treats anything with
  # only two integer values as a factor otherwise, changing rule format.
  x_train[pv] <- lapply(x_train[pv], function(col) {
    if (is.logical(col)) as.numeric(col)
    else if (is.integer(col) && length(unique(col)) == 2) as.numeric(col)
    else col
  })

  cat(sprintf("  C50_TRIALS = %d | C50_CF = %.2f | C50_MIN_CASES = %d\n",
              C50_TRIALS, C50_CF, C50_MIN_CASES))
  ...
  # ── rule extraction ───────────────────────────────────────────────────────────
  rules_raw <- parse_c50_rules(fit, positive_class = "error")
  cat(sprintf("  raw rules extracted: %d\n", nrow(rules_raw)))

  if (nrow(rules_raw) == 0) {
    cat("  No rules extracted; try lowering C50_CF or increasing C50_TRIALS\n")
    return(empty)
  }

  rules <- rules_raw %>%
    mutate(rule_id = paste0("R", seq_len(n())), coefficient = confidence)
