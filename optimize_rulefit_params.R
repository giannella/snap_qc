...
#
# It sweeps on ONE dataset (set SWEEP_HH to "all" or a single household size),
...
SWEEP_HH      <- "all"         # "all" (pooled, keeps HH size as a feature) or "1","2-3","4+"
...
MAXDEPTH_VALS  <- c(4L)
LEARNRATE_VALS <- c(0.001, 0.005, 0.01, 0.02)
...
if (SWEEP_HH != "all") {
  gtr <- hh_group_of(train_src[[HH_SIZE_COL]]); train_src <- train_src[!is.na(gtr) & gtr == SWEEP_HH, , drop = FALSE]
  gev <- hh_group_of(eval_src[[HH_SIZE_COL]]);  eval_src  <- eval_src[!is.na(gev) & gev == SWEEP_HH, , drop = FALSE]
}
...
# predictors: keep HH size only when pooling ("all"); drop it within a single stratum
tg_tr <- make_target(train_src); ie_tr0 <- tg_tr$ie; ed_tr0 <- tg_tr$ed
train_src$.is_error <- ie_tr0
pv <- if (SWEEP_HH == "all") features else setdiff(features, HH_SIZE_COL)
...
cat(sprintf("Sweep on HH=%s | train %d rows (%d errors) | eval %d rows (%d errors) | %d predictors\n",
            SWEEP_HH, nrow(train_data), sum(ie_tr), nrow(eval_src), sum(ie_ev), length(pv)))
...
sub  <- sprintf("HH = %s | trained %s | scored %s | penalty %s", SWEEP_HH,
...
# - To sweep a single stratum instead of the pooled data, set SWEEP_HH (e.g. "1", "2-3", "4+").
