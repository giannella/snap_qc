...
**I don't have a flagging system and want to identify which cases are most likely to have an error** → `INCL_find_inclusion_rules_by_hh_size_v2.R` to mine the rule pools, then `methods/build_blended_delivery_list_v2.R` to turn them into a deployable ranked list for a state: it blends the state's own mined rules into the national pool on a common confidence scale, then sizes the list to a 5% or 10% review budget against the state's own caseload — including ranked buffer rules so the workload lands on budget even as patterns drift. The state applies rules in order until reviewers are full; no outcome data is needed at any step. Ready-built lists for the states we have run so far are in `state_delivery_lists/` (see its README for column definitions and caveats). If you want to keep it very simple, start from the national rule lists in `inclusion_rules_by_hh_size_v2/`. Per-state threshold tuning (`state_threshold_gridsearch_v2.R`) remains available, but in an 18-state future-year test it did not beat deploying the national ranking as-is for the median state — treat tuning as a fallback and validate it on your own hold-out year first.
...
| `methods/build_blended_delivery_list_v2.R` (+ `methods/run_blended_delivery_batch.R`) | builds a state's deployable ranked list: blends the state's own mined rules into the national pool on the confidence-bound scale, sized to a review budget with buffer rules |
