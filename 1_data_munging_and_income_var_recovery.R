...
reg_model_data <- bind_rows(errors, no_errors)
...
#look at how many counted errors by state before subsetting to errors at cert, which will also exclude NA values for timeper1  
table(reg_model_data$state[reg_model_data$over_threshold==1])

####note that given the above, there may be errors that you care about that do not have a timeper1 value. ####
#if you want to model all errors, even at the expense of including some that are not at cert
#reg_model_data <- reg_model_data %>%
...
reg_model_data <- subset(reg_model_data, !(status=="amount correct" & total_error_amount>0))
...
#some variables that summarize hh situations
reg_model_data <- reg_model_data %>% mutate(earned_by_hh_size = recovered_fsearn / cert_HH_size_FS_n, 
                                            ...
                                            shelter_to_gross_income_ratio = as.integer(as.numeric(shelter_expenses) / as.numeric(recovered_fsearn + recovered_fsunearn + 1)))


# now we'll add some variables to reflect complexity- like number of income sources and number of deductions
...
reg_model_data$count_divisible_by_10 <- rowSums(
  sapply(reg_model_data[, tolower(all_vars)], function(x) x > 0 & x %% 10 == 0),
  na.rm = TRUE
)

reg_model_data$elderly_or_disabled_i <-reg_model_data$elderly_i | reg_model_data$fsndis>0 
