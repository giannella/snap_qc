...
#
# This reuses the grid-search logic of INCL_optimize_single_inclusion_rule_by_hh_size_for_a_state.R, but
# instead of one hand-pasted rule it reads the whole high-precision rule list that
# INCL_find_inclusion_rules_multi_model_by_hh_size.R wrote
# (final_by_HHsize_inclusion_rules_highprecision.csv, in the "inclusion_rules_by_hh_size" folder). For each rule it:
#
...
state_data <- reg_model_data %>% filter(state %in% c("Michigan") & fiscal_year>2019)

# Rule list(s) written by INCL_find_inclusion_rules_multi_model_by_hh_size.R. Point this at the file you want to optimize.
#RULES_CSV <- file.path("inclusion_rules_by_hh_size",
...
#   "precision" : maximize precision, subject to recall >= RECALL_FLOOR AND
#                 n_flagged >= MIN_FLAGGED. (Tightens rules. MIN_FLAGGED is what
#                 stops the optimizer collapsing onto a tiny high-precision sliver.)
#   "recall"    : among combos that HOLD precision >= PRECISION_TARGET (and still
...
MAX_COMBOS   <- 5000   # cap total combinations per rule (grids are thinned to fit)
...
optimize_rule <- function(rule_string, rule_id, hh_label, dat, is_error, err_dollars, csv_prec) {
  ...
  if (length(grid_idx) == 0) {
    ...
    rc <- perfs[[recall_col]]; nf <- perfs$n_flagged; pr <- perfs$precision
    
    # Shared feasibility: clear the recall floor AND flag at least MIN_FLAGGED cases.
    feas <- !is.na(rc) & rc >= RECALL_FLOOR & !is.na(nf) & nf >= MIN_FLAGGED
    
    if (OPTIMIZE_FOR == "recall") {
      # Hold precision at/above the target, then take the highest-recall combo.
      feas     <- feas & !is.na(pr) & pr >= PRECISION_TARGET
      feas_idx <- which(feas)
      if (length(feas_idx) == 0) return(NULL)
      best_i   <- feas_idx[which.max(ifelse(is.na(rc[feas_idx]), -Inf, rc[feas_idx]))]
    } else {
      # "precision": highest-precision combo among the recall/min-flagged-feasible set.
      feas_idx <- which(feas)
      if (length(feas_idx) == 0) return(NULL)
      best_i   <- feas_idx[which.max(ifelse(is.na(pr[feas_idx]), -Inf, pr[feas_idx]))]
    }
    best      <- perfs[best_i, ]
  ...
  if (is.na(best$n_flagged) || best$n_flagged < MIN_FLAGGED) return(NULL)
  prec_gate <- if (OPTIMIZE_FOR == "recall") max(PRECISION_FLOOR, PRECISION_TARGET) else PRECISION_FLOOR
  if (is.na(best$precision) || best$precision < prec_gate) return(NULL)
  ...
  tibble(
    ...
    optimize_for = OPTIMIZE_FOR, min_flagged_req = MIN_FLAGGED,
