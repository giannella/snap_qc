...
# Tree depth by approach: shallower for the stratified strata, deeper for the pooled
# model (which has hh_size as an extra predictor to split on). Overrides RF$maxdepth.
STRAT_MAXDEPTH  <- 3L
POOLED_MAXDEPTH <- 4L

MIN_STRATUM <- 10      # skip a hold-out stratum smaller than this in the small multiples
out_dir <- "byhh_vs_pooled_compare_strat2"
...
mine_rules <- function(df, drop_hh = TRUE) {
  ...
  this_maxdepth <- if (drop_hh) STRAT_MAXDEPTH else POOLED_MAXDEPTH
  
  cat(sprintf("  [%s] predictors = %d | rows = %d | errors = %d | maxdepth = %d\n",
              if (drop_hh) "stratified" else "pooled", length(pv), nrow(md), sum(ie), this_maxdepth))
  if (length(pv) < 2) {
  ...
  fit <- tryCatch(
    pre(
      ...
      ntrees = RF$ntrees, maxdepth = this_maxdepth, learnrate = RF$learnrate, type = "rules",
...
p1 <- ggplot(overall, aes(x, precision, color = approach)) +
  ...
  labs(x = xlab, y = "Precision (flagged that are errors)", color = NULL,
       title = "Stratified vs pooled rules - depth 3 by HH size, depth 4 for combined model - earned overissuance",
...
p2 <- ggplot(delta, aes(x, delta)) +
  ...
  labs(x = xlab, y = "Δ precision (by-HH − pooled)",
       title = "Precision gap from stratifying - depth 3 by HH size, depth 4 for combined model - earned overissuance",
       caption="RuleFit algorithm predicting error dollars; ntrees=10,000 (rpart), depth=4, sampfrac=.25, learnrate=.005") +
...
if (!is.null(per_hh) && nrow(per_hh) > 0) {
  p3 <- ggplot(per_hh, aes(x, precision, color = approach)) +
    ...
    scale_color_manual(values = cols) +
    labs(x = xlab, y = "Precision", color = NULL, title = "By household size (hold-out); depth 3", 
         caption="RuleFit algorithm predicting error dollars; ntrees=10,000 (rpart), depth=3, sampfrac=.25, learnrate=.005") +
