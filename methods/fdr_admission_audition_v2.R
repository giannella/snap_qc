...
cat(sprintf("national raw: %d candidates | admitted prod %d, fdr10 %d, fdr05 %d, fdr10f %d\n",
            nrow(natl), sum(natl$adm_prod), sum(natl$adm_fdr10),
            sum(natl$adm_fdr05), sum(natl$adm_fdr10f)))

ARMS <- c("prod", "fdr10", "fdr05", "fdr10f")
res <- list()
for (target in TARGETS) {
  own <- load_raw(gsub("[^A-Za-z]", "", target))
  cat(sprintf("%-22s raw %6d | prod %5d fdr10 %5d fdr05 %5d fdr10f %5d\n",
              target, nrow(own), sum(own$adm_prod), sum(own$adm_fdr10),
              sum(own$adm_fdr05), sum(own$adm_fdr10f)))
  ...
  saveRDS(bind_rows(res), file.path(out_dir, "fdr_admission_partial.rds"))
}
out <- bind_rows(res)
write.csv(out, file.path(out_dir, "fdr_admission_audition.csv"), row.names = FALSE)
cat(sprintf("wrote %s (%d rows)\n",
            file.path(out_dir, "fdr_admission_audition.csv"), nrow(out)))
