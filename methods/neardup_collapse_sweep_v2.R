# Near-duplicate collapse sweep (option C of the 2026-07-15 discussion):
# before ranking, collapse rules whose TRAIN coverage on the target state is
# nearly identical (Jaccard >= threshold), keeping the HIGHER-SUPPORT member --
# support is the honesty signal within a near-duplicate family, the LCB the
# luck-amplifier. Tests whether displacement (luck-tilted typed variants
# outranking honest pooled versions and absorbing their coverage) explains the
# five-frame vocabulary's budget-fill loss, and whether collapse repairs it.
#
# EXPLORATORY: scored on 2024 like stringency_vocabulary_sweep_v2.R; adopting
# a winner requires the honest validation steps before it ships.
#
# Arms as in the stringency sweep (orig / pooled / typed / all5, from cached
# pools -- no re-mining). z fixed at 2.326, support floor 30 (the production
# recipe); JACCARD in {1 = no collapse, 0.95, 0.9, 0.85}; collapse applied to
# the top K_TOP of each ranking (the only region the budget fill ever reads).
#
# Also reports a deployed-rule AUTOPSY per configuration: median train
# support, median raw train precision, and median train->2024 per-rule
# precision deflation of the rules the 2024 walk actually deploys.
#
# Expects `reg_model_data`. Output:
#   methods/state_similarity_v2/transfer_benchmark_train2223_test24/
#     neardup_collapse_sweep.csv

suppressMessages(library(dplyr))
source("rule_mining_helpers.R")

TARGETS <- c("Massachusetts", "Michigan", "North Carolina", "Connecticut",
             "Arizona", "Washington",
             "Louisiana", "Virginia", "California", "Texas",
             "Mississippi", "New Jersey", "Colorado",
             "Maine", "Maryland", "Missouri",
             "District of Columbia", "Tennessee")
BUDGETS <- c(0.05, 0.10)
BUFFER_MULT <- 3
LCB_Z <- 2.326
N_FLOOR <- 30
JACCARD <- c(1, 0.95, 0.9, 0.85)
K_TOP <- 3000
TRAIN_YEARS <- c("2022", "2023")
TEST_YEAR   <- "2024"

out_dir  <- "methods/state_similarity_v2/transfer_benchmark_train2223_test24"
CACHE5   <- file.path(out_dir, "pool_cache_5frames")
CACHEOLD <- file.path(out_dir, "pool_cache")

features <- c(
  "HH_size_n", "children_i", "elderly_disabled_i", "total_deductions_by_hh_size",
  "expedited_i", "cat_elig", "rawben_rel_max", "medical_deductions",
  "shelter_expenses_by_hh_size", "utilities", "married", "homeless",
  "rawearn_by_hh_size", "rawunearn_by_hh_size", "rawgross_by_hh_size",
  "percent_abawd", "unc_rawben_rel_max",
  "months_since_cert_n", "count_divisible_by_100"
)
HH_LEVELS <- c("1", "2-3", "4+")
hh_group_of <- function(n) {
  n <- suppressWarnings(as.numeric(as.character(n)))
  ifelse(is.na(n), NA_character_, ifelse(n <= 1, "1", ifelse(n <= 3, "2-3", "4+")))
}
targets_of <- function(df) {
  ie <- !is.na(df$over_threshold) & df$over_threshold != 0
  amt <- df$total_error_amount; amt[is.na(amt)] <- 0
  list(ie = ie, ed = ifelse(ie, abs(amt), 0))
}

adf <- prep_features(reg_model_data %>%
                       filter(fiscal_year %in% c(TRAIN_YEARS, TEST_YEAR)),
                     features)$data
yr <- as.character(adf$fiscal_year)
st <- as.character(adf$state)

sig <- gsub("[^A-Za-z]", "", paste(sort(unique(st)), collapse = "_"))
nat_key_old <- if (nchar(sig) <= 80) sig else
  sprintf("%s_%08x", substr(sig, 1, 60),
          sum(utf8ToInt(sig) * seq_along(utf8ToInt(sig))) %% .Machine$integer.max)

natl5   <- readRDS(file.path(CACHE5, "pool_national.rds"))
natlold <- readRDS(file.path(CACHEOLD, sprintf("pool_%s.rds", nat_key_old)))
natlold$engines <- NA_character_; natlold$mined_frames <- "any_error"

TYPED <- c("earned_overissuance", "unearned_overissuance",
           "underissuance", "other_error")
has_frame <- function(pool, frames) {
  vapply(strsplit(pool$mined_frames, "+", fixed = TRUE),
         function(f) any(f %in% frames), logical(1))
}
lcb_of <- function(pool, z) {
  k <- round(pool$precision_train * pool$n_flagged_train)
  wilson_lcb(k, pool$n_flagged_train, z)
}

# collapse near-duplicates among the top-K of `ord` (indices into pool rows,
# best first): within a stratum, rules whose train-coverage Jaccard >= jac
# form clusters (union-find); every cluster keeps only its max-support member.
# Returns the row indices to DROP.
neardup_drop <- function(pool, idx_tr, ord, jac) {
  if (jac >= 1) return(integer(0))
  top <- ord[seq_len(min(K_TOP, length(ord)))]
  drop <- integer(0)
  for (h in unique(pool$hh[top])) {
    cand <- top[pool$hh[top] == h]
    sz <- lengths(idx_tr[cand])
    keepable <- sz > 0
    cand <- cand[keepable]; sz <- sz[keepable]
    m <- length(cand)
    if (m < 2) next
    o <- order(sz)
    cand <- cand[o]; sz <- sz[o]
    uf <- seq_len(m)
    find <- function(x) { while (uf[x] != x) { uf[x] <<- uf[uf[x]]; x <- uf[x] }; x }
    for (i in seq_len(m - 1)) {
      si <- sz[i]; ii <- idx_tr[[cand[i]]]
      for (j in (i + 1):m) {
        if (sz[j] * jac > si) break   # size ratio bound: J >= jac impossible
        inter <- length(intersect(ii, idx_tr[[cand[j]]]))
        if (inter / (si + sz[j] - inter) >= jac) {
          ri <- find(i); rj <- find(j)
          if (ri != rj) uf[ri] <- rj
        }
      }
    }
    roots <- vapply(seq_len(m), find, integer(1))
    for (r in unique(roots)) {
      mem <- cand[roots == r]
      if (length(mem) < 2) next
      keep <- mem[order(-pool$n_flagged_train[mem],
                        -pool$lcb[mem], pool$rule[mem])][1]
      drop <- c(drop, setdiff(mem, keep))
    }
  }
  drop
}

walk_eval <- function(pool, idx_tr, idx_te, n_tr_rows, n_te_rows, tg_te, b) {
  cap <- floor(b * n_tr_rows); cap_buf <- floor(BUFFER_MULT * b * n_tr_rows)
  un <- rep(FALSE, n_tr_rows); n_in <- 0L
  frozen <- integer(0); buffer <- integer(0)
  ord <- order(-pool$lcb)
  for (i in ord) {
    add <- sum(!un[idx_tr[[i]]])
    if (add == 0) next
    if (n_in + add <= cap) {
      un[idx_tr[[i]]] <- TRUE; n_in <- n_in + add; frozen <- c(frozen, i)
    } else if (n_in + add <= cap_buf) {
      un[idx_tr[[i]]] <- TRUE; n_in <- n_in + add; buffer <- c(buffer, i)
    }
  }
  cap24 <- floor(b * n_te_rows)
  un24 <- rep(FALSE, n_te_rows); used <- integer(0)
  for (i in c(frozen, buffer)) {
    add <- sum(!un24[idx_te[[i]]])
    if (add > 0 && sum(un24) + add <= cap24) {
      un24[idx_te[[i]]] <- TRUE; used <- c(used, i)
    }
  }
  nb <- sum(un24)
  # deployed-rule autopsy: per-rule realized 2024 precision vs train
  n_te_r <- lengths(idx_te[used])
  k_te_r <- vapply(idx_te[used], function(ix) sum(tg_te$ie[ix]), numeric(1))
  p_te_r <- ifelse(n_te_r > 0, k_te_r / n_te_r, NA_real_)
  defl <- pool$precision_train[used] - p_te_r
  list(row = data.frame(
    n_deployed = length(used),
    workload = round(nb / n_te_rows, 4),
    precision = round(ifelse(nb > 0, sum(tg_te$ie[un24]) / nb, NA), 4),
    dollar_recall = round(sum(tg_te$ed[un24]) / sum(tg_te$ed), 4),
    dep_med_support = median(pool$n_flagged_train[used]),
    dep_med_train_prec = round(median(pool$precision_train[used]), 4),
    dep_med_deflation = round(median(defl, na.rm = TRUE), 4)))
}

res <- list()
for (target in TARGETS) {
  own5   <- readRDS(file.path(CACHE5, sprintf("pool_%s.rds", gsub("[^A-Za-z]", "", target))))
  ownold <- readRDS(file.path(CACHEOLD, sprintf("pool_%s.rds", gsub("[^A-Za-z]", "", target))))
  ownold$engines <- NA_character_; ownold$mined_frames <- "any_error"
  cols <- c("hh", "rule", "mined_frames", "n_flagged_train", "precision_train")
  pool5 <- bind_rows(natl5[, cols], own5[, cols])
  poolo <- bind_rows(natlold[, cols], ownold[, cols])

  tr <- adf[st == target & yr %in% TRAIN_YEARS, , drop = FALSE]
  te <- adf[st == target & yr == TEST_YEAR, , drop = FALSE]
  tg_te <- targets_of(te)
  strata_tr <- lapply(setNames(nm = HH_LEVELS), function(h)
    which(hh_group_of(tr$cert_HH_size_FS_n) %in% h))
  strata_te <- lapply(setNames(nm = HH_LEVELS), function(h)
    which(hh_group_of(te$cert_HH_size_FS_n) %in% h))
  idx5_tr <- flags_for_rules(pool5, tr, strata_tr, label = "")
  idx5_te <- flags_for_rules(pool5, te, strata_te, label = "")
  idxo_tr <- flags_for_rules(poolo, tr, strata_tr, label = "")
  idxo_te <- flags_for_rules(poolo, te, strata_te, label = "")

  arms <- list(
    orig   = list(pool = poolo, tr = idxo_tr, te = idxo_te,
                  sub = rep(TRUE, nrow(poolo))),
    pooled = list(pool = pool5, tr = idx5_tr, te = idx5_te,
                  sub = has_frame(pool5, "any_error")),
    typed  = list(pool = pool5, tr = idx5_tr, te = idx5_te,
                  sub = has_frame(pool5, TYPED)),
    all5   = list(pool = pool5, tr = idx5_tr, te = idx5_te,
                  sub = rep(TRUE, nrow(pool5)))
  )
  for (arm in names(arms)) {
    a <- arms[[arm]]
    keep0 <- which(a$sub & a$pool$n_flagged_train >= N_FLOOR)
    p <- a$pool[keep0, , drop = FALSE]
    p$lcb <- lcb_of(p, LCB_Z)
    # blend dedup: identical rule text keeps the higher-bound version
    o <- order(-p$lcb)
    dup <- duplicated(paste(p$hh[o], p$rule[o], sep = "\r"))
    sel <- o[!dup]
    p2 <- p[sel, , drop = FALSE]
    k2 <- keep0[sel]
    itr <- a$tr[k2]; ite <- a$te[k2]
    ord2 <- order(-p2$lcb)
    for (jac in JACCARD) {
      dr <- neardup_drop(p2, itr, ord2, jac)
      ok <- setdiff(seq_len(nrow(p2)), dr)
      p3 <- p2[ok, , drop = FALSE]
      for (b in BUDGETS) {
        ev <- walk_eval(p3, itr[ok], ite[ok], nrow(tr), nrow(te), tg_te, b)
        res[[length(res) + 1]] <- cbind(
          data.frame(target = target, arm = arm, jaccard = jac, budget = b,
                     n_collapsed = length(dr),
                     target_base_rate = round(mean(tg_te$ie), 4)), ev$row)
      }
    }
    cat(sprintf("%-22s %s done\n", target, arm))
  }
  saveRDS(bind_rows(res), file.path(out_dir, "neardup_sweep_partial.rds"))
}
out <- bind_rows(res)
write.csv(out, file.path(out_dir, "neardup_collapse_sweep.csv"), row.names = FALSE)
cat(sprintf("wrote %s (%d rows)\n",
            file.path(out_dir, "neardup_collapse_sweep.csv"), nrow(out)))

cat("\nmedians by arm x jaccard (precision / dollar recall / deployed support / deflation):\n")
sm <- out %>% group_by(arm, jaccard, budget) %>%
  summarise(med_prec = median(precision, na.rm = TRUE),
            med_dollars = median(dollar_recall, na.rm = TRUE),
            med_dep_support = median(dep_med_support, na.rm = TRUE),
            med_deflation = median(dep_med_deflation, na.rm = TRUE),
            .groups = "drop") %>%
  arrange(budget, desc(med_prec))
print(as.data.frame(sm), row.names = FALSE)
