...
> our own rule generator gave the same rule quality at a fraction of the cost (about a
> fifth of the compute, and a >40 GB memory peak brought down to a few GB), and it
> made analyses possible that `pre` simply could not fit (the
> pooled any-error model, 853k-rule comparisons, a regression test). Worth knowing:
> swapping the underlying tree engines barely moved precision (~1 point) — the real
> gains come from strict filtering and from scoring on any-error, not from the choice
> of algorithm.

The head-to-head, everything else held equal:

- **Quality is a wash.** On the earned frame, `pre` kept 68 rules at median holdout
  precision 0.134; our generator kept 29 at 0.157 — the same ballpark, with a fifth
  of the trees.
...
- **Slow learning, low row-sampling.** A slow learning rate helped (eta 0.02 → 0.217
  precision vs eta 0.1 → 0.212), and a low row-sample beat a high one (subsample 0.20
  → 0.218 vs 0.75 → 0.208); within 0.15-0.30 the choice does not matter, so production
  uses 0.20. (Honest footnote: the "low beats high" edge did *not* replicate on a
  later test year — see section 13 — so treat 0.20 as "as good as anything," not
  proven best.)
- **Deeper trees help the forest, not the booster.** Depth 5 beat depth 3 clearly for
  ranger (0.203 → 0.213) but only marginally for xgboost (0.210 → 0.211); production
  uses depth 4.
...
- Concretely, a big pool at the 99% bound and a small pool at the 90% bound land on
  the *same* operating point (≈55% recall at 17% precision) — but the big pool has
...
trees (mtry 2), 99% bound — about 1,535 rules kept, with better median holdout
...
pattern was sharp — it depended on how much of its own data a state had:
...
public file can even show — frame errors ÷ (all over-threshold errors + ineligible
exclusions), FY22-24. Nationally it is 71%; the best-covered states reach 78-81%. The
floor is set by *ineligible cases*, which the public file omits entirely and which
are 100%-of-benefit errors: New Jersey sees just 43% of its errors, Tennessee 51%,
several states ~53%. Below ~60%, public rules are only a supplement — the state
should mine its own internal data, which contains those ineligible determinations.
...
similar-state pool) — more data. At a 10% budget the small similar-state pools looked
competitive or better in about half the states (best pool 0.278 vs the national
pool's 0.245).
...
- **Big ensembles widen the menu, not the frontier:** replicated — ~7x more rules at
  a nearly identical frontier.
...
cases removed — lands essentially identically, 0.296 at 5% and 0.276 at 10%, so
...
The state walks the list in rank order, turning on rules until capacity fills — no
...
We put each state's own mined rules and the national rules on one scale — every rule
ranked by its own 99% Wilson bound (national rules bounded on national data, state
rules on the state's own). Because both bounds mean "at least this precise, with 99%
confidence," the merged ranking is coherent and it automatically discounts
...
(48k → 159k nationally) but *lowered* budget-filled precision on the 2024 test (0.306
...
## 19. Admission: FDR + support floor validated on two eras; floorless FDR refuted

> **Takeaway — about our pipeline (with a portable statistical lesson).** A standard
> multiple-testing gate (Benjamini-Hochberg controlling the false-discovery rate at
> 10%) plus the n >= 30 support floor matched our hand-tuned admission filter at the
> 5% budget on two eras (and was never worse), with a smaller admitted pool — so that
> is now the principled default.
> The lesson that generalizes: multiplicity control ("is this rule real?") and a
> support floor ("is the estimate reliable enough to rank on?") are different jobs;
> dropping the floor let small-sample rules collapse the results.

On the raw, unfiltered rule vocabularies (~145k candidates per era), a standard
multiple-testing gate — Benjamini-Hochberg controlling the false-discovery rate at
10% against the stratum base rate — *plus* the n >= 30 support floor matched our
hand-tuned admission filter exactly at a 5% budget on both test eras, using a ~40%
smaller admitted pool. So it replaces the hand-tuned filter as a principled default.

The instructive part is what happens *without* the floor: floorless Benjamini-Hochberg
collapsed (0.284 vs 0.335 at a 5% budget). Multiplicity control answers "is this rule
real?"; the support floor answers "is its estimate steady enough to *rank* on?" They
are different jobs, and you need both. (The gate also self-scales: it admitted 54k of
145k rules nationally but only a few hundred for small states — no hand-set constant.)
...
half they were mined from, versus a bound computed on the *untouched* other half —
...
budget. The curse lives in the extreme top of the list — which is exactly where a
tight review budget operates.
