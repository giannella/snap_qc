...
| `utilities_sua` (3-level SUA tier: 0 none / 1 below state-year anchor - 200 / 2 at or above; anchor = mode of positive utilities per state-year, features.R add_sua_tier) replacing `utilities` in the mined vocabulary: one-year-ahead the family's held-out reach-collapse falls from ~3.0x the non-utilities reference to 0.16x (3 of 1,430 in-band rules), list medians non-inferior (precision +0.0000 at both budgets; dollar recall -0.0032 at 5%, +0.0125 at 10%), harmed tail (4 / 1) inside the same-recipe re-mine noise; family share of the admitted pool 10.1% vs 26.9% raw | option (PROMOTED INTO THE FRAME 2026-08-22 as a v1 column; SHIPPED as the delivery lists 2026-08-23, with the anchor redefined in the same change to the state's published heating/cooling standard, additional_data/state_sua.csv max_sua, no band; era-2 replication under the new definition pending) | one era (mined FY2022-23, walked FY2024), 49 states, single seed; anchor chosen on outcome-free descriptives, the mode-vs-max translation test read the same holdout | methods/v250_benchmark_2024_utilrel/result_2026-08-22.md (findings section pending) |
| The $200 HIGH band in `utilities_sua` is a v1 engineering parameter (captures household-size / regional SUA variants: VA 375/476, NY 877/988/1,062; empty in most state-years, 10-47% in multi-schedule states), not a studied claim; proportionally looser where the anchor is low | superseded 2026-08-23: the external-anchor redefinition (state_sua.csv) removed the band | band census, 147 state-years | methods/vocab_hygiene_census/utilities_high_band_census.csv |
