...
- **08-04**: Profiled what each delivered rule actually catches (#29, issue #2):
  the modal error element is era-stable (agrees for 87.5% of rules against 41.6%
  chance) but describes a median 34.9% of a rule's errors, and 320 of 529 rules
  share the same one. A review-mode indicator puts 494 of 529 rules in one
  bucket. Two of three promotion criteria fail; not promoted to the lists. All
  98 state-and-budget combinations reproduced the shipped scorecard.
...
## 29. Per-rule error profiles: era-stable, but not concentrated and not discriminating (2026-08-04)
...
Two bases, computed separately and reported separately. **national**: the rule
evaluated against every national row of the era within its household-size stratum, so
both eras are computed identically and are comparable; used for all three criteria.
**deployed**: the FY2024 cases the delivered lists actually pull through that rule; used
for the decision-support figures. Deployed cells are thin, a median of 4 error cases and
5 variances per rule, with 80 of 543 rules catching no error at all in their deployed
cases, which is why the criteria run on the national basis.

Train era FY2022-23, test era FY2024, a true future year for the lists (built on
FY2022-24 vocabulary but ranked and filled on the training caseload). 2,390 deployed
rule instances across 98 lists, 543 distinct stratum-and-rule pairs, 12,869 flagged case
rows. 60,055 variance records join to a frame row on (FIPS, YRMONTH, HHLDNO) within
fiscal year, covering 13,283 of the 13,288 error cases in the frame.
...
| deployed, FY2024 (n >= 5 variances) | 0.412 | | |

### Criterion 2: era-stability

Modal element agrees for 463 of 529 rules (0.875). Chance agreement from the FY2024
marginal is 0.416, so agreement above chance is 0.786 on a kappa scale. Continuous axes
across eras: catchable-at-action 0.580, desk-closable 0.500, agency-caused 0.411,
pre-authorization 0.283.

### Criterion 3: discrimination

Modal element across the 529 characterisable rules on FY2024:
...
Review mode is assigned as pre_authorization when at least half a rule's variances were
catchable at or before the agency's action AND closable from the case record AND
agency-caused; post_authorization below a quarter; mixed between; insufficient under 5
variances.
...
### Levels, and what is not a partition

AGENCY, DISCOV, VERIF, TIMEPER and ELEMENT are recorded per variance and a case carries
up to nine, so every figure is variance-level unless it says cases. A case also trips
several rules, so per-rule counts sum to more than the flagged total.

### Caveats

Code definitions come from the FY2023 SNAP QC Technical Documentation (Mathematica),
Chapter V detailed codebook; the .sav files carry no value labels for these variables.
AGENCY codes 22 to 26 are documented nowhere through FY2023 and are bucketed
UNDOCUMENTED; code 26 alone carries 5,669 of 20,769 populated FY2024 values (27.3%), so
every cause figure is computed over the documented remainder. E_FINDG is populated for
13,532 of those 20,769 records, three states populate none, and Michigan populates 2.

The single-state version of this analysis (Michigan, FY2024, 10% budget) is reproduced
by this build: 19 rules deployed of 194, 86 cases flagged, 24 errors, precision 0.2791,
15 of 19 rules catching at least one error.

*Artifacts: `methods/rule_error_profiles/` (README.md with the full tables,
`stage1_checks.csv` for the 98 reproduction checks, `deployed_rules.csv`,
`rule_profiles.csv`, `promotion_criteria.md`). Regenerated by
