# ──────────────────────────────────────────────────────────────────────────────
# ENGINE head-to-head: bagged rpart (CART) vs xgboost vs ranger (v2 stack)
#
# Question: does the classic rpart/CART engine — {pre}'s rule generator, minus
# the boosting and lasso — hold up against xgboost and ranger when everything
# downstream (canonicalize, screen, dedup, stringent LCB filter, sweep) is
# identical? All engines at production scale: 1000 trees/rounds, depth 4.
#
#   rpart:   bagged, subsample 0.20 per tree, minbucket 20 (anova on 0/1)
#   xgboost: nrounds 1000, eta 0.02, subsample 0.20
#   ranger:  1000 trees, mtry 2, min.node.size 20
#
# Frame: any_error; strata 1 / 2-3 / 4+; z = 2.326 (production filter).
# Expects `reg_model_data`. Outputs -> methods/compare_engines_v2/.
# ──────────────────────────────────────────────────────────────────────────────
...
ENGINES <- list(
  "rpart (bagged CART)" = function(sub, ie_s)
    generate_rules_rpart(sub, ie_s, pv, num_trees = 1000, max_depth = 4,
                         sample_frac = 0.20, min_bucket = 20, seed = 117),
  "xgboost" = function(sub, ie_s)
    generate_rules_xgboost(sub, ie_s, pv, nrounds = 1000, max_depth = 4,
                           eta = 0.02, subsample = 0.20, seed = 117),
  "ranger (mtry=2)" = function(sub, ie_s)
    generate_rules_ranger(sub, ie_s, pv, num_trees = 1000, max_depth = 4,
                          mtry = 2, min_node_size = 20, seed = 117)
)

run_engine <- function(nm) {
  t0 <- Sys.time()
  rules <- bind_rows(lapply(HH_LEVELS, function(h) {
    ix <- strata_tr[[h]]
    data.frame(rule = canonicalize_rules(
      ENGINES[[nm]](train[ix, , drop = FALSE], tg_tr$ie[ix]), SIGNIF_DIGITS),
      hh = h, stringsAsFactors = FALSE)
  })) %>% distinct(rule, hh)
  mining_secs <- as.numeric(difftime(Sys.time(), t0, units = "secs"))
  cat(sprintf("\n[%s] %d canonical rules mined in %.0fs\n", nm, nrow(rules), mining_secs))
  ...
  cat(sprintf("[%s] screen+dedup -> %d rules | pass filter >= %.2f: %d\n",
              nm, nrow(rules), MIN_PRECISION,
  ...
  list(sweep = sw %>% mutate(engine = nm),
       info = data.frame(engine = nm, n_rules = nrow(rules),
                         n_pass_filter = sum(rules$stat >= MIN_PRECISION, na.rm = TRUE),
                         mining_secs = round(mining_secs), total_secs = round(total_secs)))
}

runs <- lapply(names(ENGINES), run_engine)
sweeps <- bind_rows(lapply(runs, `[[`, "sweep"))
info   <- bind_rows(lapply(runs, `[[`, "info"))
sweeps$engine <- factor(sweeps$engine, levels = names(ENGINES))
write.csv(sweeps, file.path(out_dir, "engine_sweeps.csv"), row.names = FALSE)
...
summary_tbl <- sweeps %>% group_by(engine) %>%
  ...
  left_join(info, by = "engine")
write.csv(summary_tbl, file.path(out_dir, "engine_summary.csv"), row.names = FALSE)
cat("\nEngine summary (matched dollar recall 0.02-0.50):\n")
print(as.data.frame(summary_tbl), row.names = FALSE)

cols <- c("rpart (bagged CART)" = "#d1495b", "xgboost" = "#0073b7",
          "ranger (mtry=2)" = "#2e8b57")
p <- ggplot(sweeps, aes(x, precision, color = engine)) +
  ...
  labs(x = "Hold-out dollar recall of the union (all 2023 errors)",
       ...
       title = "Rule-mining engines head-to-head: bagged rpart vs xgboost vs ranger",
       subtitle = "1,000 trees/rounds each, depth 4; identical screens, dedup and 99% Wilson-LCB filter;\nany-error frame, trained 2022/2024, scored on 2023; point labels = LCB floor") +
  theme_minimal(base_size = 12) + theme(legend.position = "top")
save_png(p, file.path(out_dir, "engine_sweeps.png"), 9, 5.5)
cat(sprintf("\nWrote engine comparison to %s/\n", out_dir))
