...
for (state in states) {
  ...
  # Plot tree
  plot_path <- paste0("state_income_error_trees_any_timeper/", state, "_reg_2017_2023_any_timeper_basic.png")
  dir.create("state_income_error_trees_any_timeper", showWarnings = FALSE, recursive = TRUE)
  
  plot_pooled_tree(
    tree_model    = tree_model,
    main_title    = paste("Regression Tree —", state),
...
kable(top5_importance, digits = 1, format = "simple")
