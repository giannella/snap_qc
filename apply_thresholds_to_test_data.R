...
#
# optimal_thresholds: output from find_optimal_underissuance_thresholds()
# test_data: new data to apply the rules to (e.g., 2023 data)

apply_optimal_thresholds_to_test <- function(optimal_thresholds,
                                              test_data,
                                              geography_var = "state",
  ...
  for (geo in geographies) {
    ...
    rawben_thresh_lower <- geo_thresholds$rawben_rel_max_thresh_lower[1]
    rawben_thresh_upper <- geo_thresholds$rawben_rel_max_thresh_upper[1]
    ...
    cat("    rawben_rel_max >=", rawben_thresh_lower, "and <", rawben_thresh_upper, "\n")
    cat("    deductions_by_hh_size <", deductions_thresh, "\n\n")
    
    # Filter test data to this geography (ALL cases, not just underissuance)
    geo_test_data <- test_data %>%
      filter(!!sym(geography_var) == geo)
    
    # Get underissuance subset for calculating total error
    underiss_test_data <- geo_test_data %>%
      filter(tolower(!!sym(error_finding_var)) == "underissuance")
    
    if (nrow(underiss_test_data) == 0) {
      cat("  No underissuance test data for this geography. Skipping.\n")
      next
    }
    ...
    cat("    Rows in underiss_test_data:", nrow(underiss_test_data), "\n")
    cat("    Non-NA error amounts:", sum(!is.na(underiss_test_data[[error_amount_var]])), "\n")
    cat("    Non-zero error amounts:", sum(underiss_test_data[[error_amount_var]] > 0, na.rm = TRUE), "\n")
    ...
    if (!"fywgt" %in% names(geo_test_data)) {
      ...
      underiss_test_data$fywgt <- 1
    }
    
    cat("    Non-NA weights:", sum(!is.na(underiss_test_data$fywgt)), "\n")
    cat("    Non-zero weights:", sum(underiss_test_data$fywgt > 0, na.rm = TRUE), "\n")
    cat("    Sum of error_amount (unweighted):", sum(underiss_test_data[[error_amount_var]], na.rm = TRUE), "\n")
    cat("    Sum of fywgt:", sum(underiss_test_data$fywgt, na.rm = TRUE), "\n")
    
    # Calculate total error in test data (weighted)
    # Only multiply where BOTH error_amount and fywgt are non-NA
    valid_rows <- !is.na(underiss_test_data[[error_amount_var]]) & !is.na(underiss_test_data$fywgt)
    total_test_error <- sum(underiss_test_data[[error_amount_var]][valid_rows] * underiss_test_data$fywgt[valid_rows])
    total_test_cases <- nrow(geo_test_data)  # All cases for precision calculation
    n_test_sig_errors <- sum(underiss_test_data[[overthreshold_var]] == 1, na.rm = TRUE)
    
    cat("    Valid rows for weighting:", sum(valid_rows), "\n")
    cat("    Calculated total_test_error:", total_test_error, "\n")
    
    cat("\n  Test data summary:\n")
    cat("    Total underissuance cases:", total_test_cases, "\n")
    ...
    flagged <- geo_test_data$earned_by_hh_size >= earned_thresh & 
               geo_test_data$rawben_rel_max >= rawben_thresh_lower &
               geo_test_data$rawben_rel_max < rawben_thresh_upper &
               geo_test_data$deductions_by_hh_size < deductions_thresh
    
    flagged[is.na(flagged)] <- FALSE
    
    # Calculate performance metrics
    # For RECALL: count both underissuance AND overissuance errors captured (weighted)
    ...
    valid_issuance_flagged <- issuance_errors_flagged & !is.na(geo_test_data[[error_amount_var]]) & !is.na(geo_test_data$fywgt)
    error_dollars_captured <- sum(geo_test_data[[error_amount_var]][valid_issuance_flagged] * geo_test_data$fywgt[valid_issuance_flagged])
    ...
    total_sig_error_weighted <- sum(geo_test_data[[error_amount_var]][sig_errors_in_geo] * geo_test_data$fywgt[sig_errors_in_geo])
    
    # For PRECISION: count all flagged cases (including correct cases, overissuance, etc.)
    # For dollars_per_case: only use SIGNIFICANT errors flagged (unweighted)
    ...
    n_false_positives <- sum((geo_test_data[[overthreshold_var]][flagged] != 1 | geo_test_data[[error_amount_var]][flagged] == 0), na.rm = TRUE)
    ...
    geo_result <- data.frame(
      ...
      rawben_rel_max_thresh_lower = rawben_thresh_lower,
      rawben_rel_max_thresh_upper = rawben_thresh_upper,
      ...
      # Test performance
      test_total_error = total_sig_error_weighted,  # Now: total significant error (weighted)
      ...
      test_dollars_per_case = ifelse(n_sig_errors_flagged > 0, error_dollars_sig_only / n_sig_errors_flagged, 0),  # Now: only significant errors
...
#   total_deductions_range = seq(0, 800, by = 100),
...
#   error_amount_var = "total_error_amount",
#   overthreshold_var = "overthreshold",
#   error_finding_var = "status_c"  # Use this if your test data uses status_c instead of e_findg1
# )
# 
...
#   print()
# 
# write.csv(test_results, "test_performance_2023.csv", row.names = FALSE)
# 
