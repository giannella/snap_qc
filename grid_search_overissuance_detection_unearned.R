...
#   - unearned_by_hh_size >= threshold (lower bound)
#   - shelter_expenses between lower and upper bounds
#   - deductions_by_hh_size <= threshold (upper bound: lower deductions → overpaid)
#   - rawben_rel_max >= threshold (lower bound)
#   - hh_size >= threshold (lower bound, integer)

generate_overissuance_grid <- function(data,
                                       error_amount_var = "total_error_amount",
                                       error_threshold = 52,
                                       unearned_by_hh_size_range    = seq(0, 600, by = 50),
                                       shelter_expenses_range     = seq(0, 1000, by = 100),
                                       deductions_by_hh_size_range = seq(0, 800, by = 100),
                                       rawben_rel_max_range        = seq(0, 1, by = 0.1),
                                       hh_size_range              = 1:6) {
  ...
  threshold_grid <- expand.grid(
    unearned_by_hh_size_thresh      = unearned_by_hh_size_range,
    shelter_expenses_thresh_lower = shelter_expenses_range,   # Lower bound
    shelter_expenses_thresh_upper = shelter_expenses_range,   # Upper bound
    deductions_by_hh_size_thresh  = deductions_by_hh_size_range,
    rawben_rel_max_thresh         = rawben_rel_max_range,
    hh_size_thresh               = hh_size_range,
    stringsAsFactors = FALSE
  ) %>%
    # Only keep combinations where lower <= upper
    filter(shelter_expenses_thresh_lower <= shelter_expenses_thresh_upper)

  cat("  Testing", format(nrow(threshold_grid), big.mark = ","), "threshold combinations...\n")

  # Calculate metrics for each threshold combination
  calc_metrics <- function(unearned_thresh, shelter_lower, shelter_upper, ded_thresh, rawben_thresh, hh_size_thresh_val, data_to_use, total_error) {

    # Flag cases that meet all thresholds:
    #   - unearned_by_hh_size  >= unearned_thresh     (lower bound)
    #   - shelter_expenses   >= shelter_lower  AND  < shelter_upper  (band)
    #   - deductions_by_hh_size <= ded_thresh     (upper bound)
    flagged <- data_to_use$unearned_by_hh_size >= unearned_thresh &
               data_to_use$shelter_expenses  >= shelter_lower &
               data_to_use$shelter_expenses  <  shelter_upper &
               data_to_use$deductions_by_hh_size <= ded_thresh &
               data_to_use$rawben_rel_max >= rawben_thresh &
               data_to_use$cert_HH_size_FS_n >= hh_size_thresh_val
  ...
  results <- threshold_grid %>%
    mutate(
      metrics = pmap(
        list(unearned_by_hh_size_thresh,
             shelter_expenses_thresh_lower,
             shelter_expenses_thresh_upper,
             deductions_by_hh_size_thresh,
             rawben_rel_max_thresh,
             hh_size_thresh),
        ~ calc_metrics(..1, ..2, ..3, ..4, ..5, ..6, filtered_data, total_error_dollars)
      )
    ) %>%
...
run_overissuance_grid_by_geography <- function(data,
  ...
  for (geo in geographies) {
    ...
    geo_results <- generate_overissuance_grid(
      ...
      unearned_by_hh_size_range     = unearned_by_hh_size_range,
      shelter_expenses_range      = shelter_expenses_range,
      deductions_by_hh_size_range = deductions_by_hh_size_range,
      rawben_rel_max_range        = rawben_rel_max_range,
      hh_size_range              = hh_size_range
    )
    ...
    if (nrow(geo_results) > 0) {
      ...
      geo_results %>%
        ...
        head(5) %>%
        select(unearned_by_hh_size_thresh,
               shelter_expenses_thresh_lower, shelter_expenses_thresh_upper,
               deductions_by_hh_size_thresh, rawben_rel_max_thresh, hh_size_thresh,
